...
const AdminInventory = () => {
  ...
  // Pie chart data
  const pieChartData = [
    { name: 'In Stock', value: inventoryItems.filter(i => i.availability).length },
    { name: 'Out of Stock', value: inventoryItems.filter(i => !i.availability).length }
  ];
          ...
          {currentItems.map((item) => (
            <div className="material-card" key={item._id}>
              ...
              <p className={`stock-status ${
                ...
                <strong>Status:</strong> {item.availability ? 'In Stock' : 'Out of Stock'}
              </p>
